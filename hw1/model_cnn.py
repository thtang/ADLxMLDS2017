...
for b in batch:
    frame_feature = []
    frame_label = []
    for index, instance_id in enumerate(b):
        temporal_feature = []
        
        if index ==0:
            temporal_feature.append(np.zeros(108))
        else:
            temporal_feature.append(np.hstack((mfcc_feature_dict[b[index-1]] , fbank_feature_dict[b[index-1]])))
            
        temporal_feature.append(np.hstack((mfcc_feature_dict[b[index]] , fbank_feature_dict[b[index]])))
...
#                 temporal_feature.append(np.zeros(108))
        ...
        frame_feature.append(np.transpose(temporal_feature))
...
model = Sequential()
model.add(TimeDistributed(Conv1D( filters=16, kernel_size=6),input_shape =(frame_count,108,3)))
...
model.fit(train_X, train_y, epochs=120, batch_size=128)
